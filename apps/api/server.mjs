...
app.post('/api/smart-home/toggle', async (req, reply) => {
  try {
    ...
    const service = ['light','switch'].includes(domain) ? 'toggle' : 'turn_on';
...
app.get('/api/homekit/summary', async (req, reply) => {
  const configured = Boolean(process.env.HOME_ASSISTANT_URL && process.env.HOME_ASSISTANT_TOKEN);
  if (!configured) {
    return { status: 'not_configured', message: 'Set HOME_ASSISTANT_URL and HOME_ASSISTANT_TOKEN in .env to enable Home Assistant integration.', entities: [] };
  }
  try {
    const base = process.env.HOME_ASSISTANT_URL.replace(/\/$/, '');
    const res = await fetch(`${base}/api/states`, { headers: { Authorization: `Bearer ${process.env.HOME_ASSISTANT_TOKEN}` } });
    if (!res.ok) return reply.code(502).send({ status: 'error', message: `Home Assistant returned ${res.status}`, entities: [] });
    const states = await res.json();
    ...
    return { status: 'ok', url: base, count: entities.length, entities };
  } catch (error) {
    return reply.code(502).send({ status: 'error', message: error.message, entities: [] });
  }
});
