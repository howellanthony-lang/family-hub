...
ensure_clean_or_reviewed() {
  if ! command -v git >/dev/null 2>&1 || ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    printf 'Not inside a git work tree; skipping git status check.\n'
    return 0
  fi

  git status --short
  if [ -n "$(git status --porcelain)" ]; then
    printf 'Working tree has local changes. Review, commit, or stash before deploy.\n'
    return 1
  fi
}
...
pull_latest() {
  if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    printf 'Not inside a git work tree; skipping git pull.\n'
  fi
}
...
main() {
  info "Family Hub Pi Deploy"
  detect_host
  run_check "working tree review" ensure_clean_or_reviewed
  run_check "git pull --ff-only" pull_latest
